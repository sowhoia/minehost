...
**macOS не даёт запустить?** Один раз, в зависимости от сообщения:

- *«Непроверенный разработчик»* (подписанные сборки) — «Системные
  настройки → Конфиденциальность и безопасность → Открыть всё равно»
  (на macOS ≤ 14 достаточно ПКМ по приложению → «Открыть»).
- *«Приложение повреждено»* (старые неподписанные сборки) — после
