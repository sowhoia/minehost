...
**macOS не даёт запустить?** Сборки начиная с v1.1.0 подписаны: при
первом запуске macOS покажет предупреждение о непроверенном
разработчике — открой «Системные настройки → Конфиденциальность и
безопасность» и нажми «Открыть всё равно» (на macOS ≤ 14 достаточно
ПКМ по приложению → «Открыть»). Это нужно один раз.

Для старых версий (≤ 1.0.1, «приложение повреждено») после установки
в «Программы» выполни в Терминале и запускай как обычно:
